          ...
          >
            ...
            <div className="space-y-3">
              {phase.items.map((item, i) => (
                <div 
                  key={i}
                  className={`bg-white/80 p-3 rounded-lg border transition-colors cursor-pointer group ${activeStep === item.title ? 'border-gray-400 shadow-sm' : 'border-white hover:border-gray-300'}`}
                  onClick={() => setActiveStep(item.title)}
                >
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-gray-700">{item.title}</span>
                    <CheckCircle2 className={`w-4 h-4 transition-opacity ${activeStep === item.title ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} ${phase.textColor}`} />
                  </div>
