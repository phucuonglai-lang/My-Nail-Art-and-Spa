      ...
      <AnimatePresence>
        {selectedPolicy && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-brand-bg/95 backdrop-blur-xl z-[200] flex items-center justify-center p-6"
          >
            <motion.div 
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-brand-card w-full max-w-2xl rounded-[48px] p-12 shadow-[0_32px_80px_rgba(0,0,0,1)] relative max-h-[90vh] flex flex-col border border-white/5"
            >
              <button 
                onClick={() => setSelectedPolicy(null)} 
                className="absolute top-10 right-10 p-3 text-white/20 hover:text-white hover:bg-white/10 rounded-full transition-all"
              >
                <X size={24} />
              </button>

              <div className="mb-10">
                <span className="text-[10px] font-bold uppercase tracking-[5px] text-brand-purple mb-3 block">Official Policy</span>
                <h3 className="text-3xl font-bold text-white uppercase tracking-tight leading-tight">
                  {selectedPolicy.title}
                </h3>
              </div>

              <div className="overflow-y-auto flex-1 prose prose-invert prose-sm max-w-none pr-4 scrollbar-hide">
                <div 
                  className="whitespace-pre-wrap text-white/50 leading-relaxed font-sans text-base"
              ...
              <div className="mt-12 pt-8 border-t border-brand-border flex justify-end">
                <button 
                  onClick={() => setSelectedPolicy(null)}
                  className="px-12 py-5 bg-gradient-to-r from-brand-accent to-brand-purple text-white rounded-2xl font-bold uppercase tracking-[4px] text-[10px] hover:shadow-[0_20px_40px_rgba(255,45,85,0.2)] transition-all hover:-translate-y-1 active:translate-y-0"
                >
                  Acknowledge
                </button>
              </div>
