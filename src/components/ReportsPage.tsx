...
const BRANCH_SHEETS = [
  ...
  },
  {
    ...
    tableData: [
      ['12/22-01/04', '$3.783,00', '$194.165,00', '$54.468,00', '$39.621,00'],
      ['01/05-01/18', '$2.500,00', '$15.000,00', '$48.438,00', '$33.000,00'],
      ['01/19-02/01', '$2.100,00', '$12.000,00', '$45.975,00', '$31.000,00'],
      ['02/02-02/15', '$3.200,00', '$18.000,00', '$51.356,00', '$35.000,00'],
      ['02/16-03/01', '$3.500,00', '$20.000,00', '$55.011,00', '$38.000,00'],
      ['03/02-03/15', '$3.300,00', '$19.000,00', '$53.322,00', '$36.000,00'],
      ['03/16-03/29', '$3.600,00', '$22.000,00', '$56.081,00', '$39.000,00'],
      ['03/30-04/12', '$2.900,00', '$16.000,00', '$49.507,00', '$34.000,00'],
      ['04/13-04/26', '$4.100,00', '$25.000,00', '$60.681,00', '$42.000,00'],
      ['04/27-05/10', '$4.200,00', '$26.000,00', '$61.497,00', '$43.000,00']
    ]
  }
];
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button 
                onClick={() => {
                  ...
                  const dataRows = (activeBranch as any)?.tableData || [];
                  let csvContent = "data:text/csv;charset=utf-8," 
                    + headers.join(",") + "\n"
                    + dataRows.map((e: any) => e.join(",")).join("\n");
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody className="text-white/80 text-[11px] font-medium">
                  <tr className="bg-white/5 font-bold text-white">
                    ...
                    <td className="p-3 border border-white/10 text-emerald-400">{activeBranch?.stats?.revenue ? '$8.510,00' : '---'}</td>
                    <td className="p-3 border border-white/10 text-emerald-400">{activeBranch?.stats?.profit || '---'}</td>
                    <td className="p-3 border border-white/10">{activeBranch?.stats?.revenue || '---'}</td>
                    <td className="p-3 border border-white/10 text-rose-400">{activeBranch?.stats?.salary || '---'}</td>
                  </tr>
                  {((activeBranch as any)?.tableData || []).map((row: any, i: number) => {
                    const isSpecial = row[0].toLowerCase().includes('chia lợi nhuận');
                    return (
                      <tr key={i} className={cn("hover:bg-white/5 transition-colors", isSpecial && "bg-rose-500/20")}>
                        <td className={cn("p-3 border border-white/5 font-mono", isSpecial && "text-rose-500 font-bold")}>{row[0]}</td>
                        <td className={cn("p-3 border border-white/5 text-emerald-500/80", isSpecial && "text-rose-500 font-bold")}>{row[1]}</td>
                        <td className={cn("p-3 border border-white/5 text-emerald-500/80", isSpecial && "text-rose-500 font-bold")}>{row[2]}</td>
